...
; add the two integers, and display the sum of the two integers.

.model small                      ; Define a small memory model
.stack 100h                       ; Allocate 256 bytes of stack space
.data                             ; Begin data segment
    prompt1         db 'Enter first addend: $'          ; Prompt for first number input
    prompt2         db 13, 10,'Enter second addend: $'  ; Prompt for second number input with line breaks
    result_msg      db 13, 10, 10,'Sum: $'              ; Message to display before the result
    ...
    sum             dw ?                        ; Variable to store the sum of the two numbers
...
main proc                         ; Main procedure starts
    ...
    ; Display sum
    mov ax, sum                   ; Move the sum into AX
    call print_number             ; Call procedure to print the sum
